  ...
  container_definitions = jsonencode([
    {
      ...
      # Secrets are pulled from SSM Parameter Store at task start
      secrets = [
...
resource "aws_ecs_service" "blue" {
  ...
  lifecycle {
    # Pipeline manages task_definition and desired_count at runtime
...
resource "aws_ecs_service" "green" {
  ...
  cluster         = aws_ecs_cluster.main.id
  task_definition = aws_ecs_task_definition.app.arn
  desired_count   = 0  # Pipeline scales this up during deployment
  launch_type     = "FARGATE"
...
output "ecs_cluster_name" {
  description = "Use as ECS_CLUSTER in GitHub Actions variables"
  value       = aws_ecs_cluster.main.name
}

output "blue_service_name" {
  description = "Use as BLUE_SERVICE in GitHub Actions variables"
  value       = aws_ecs_service.blue.name
}

output "green_service_name" {
  description = "Use as GREEN_SERVICE in GitHub Actions variables"
