...
# Extra permissions: SSM Parameter Store, Secrets Manager, ECR, CloudWatch Logs
...
# ── Task Role (the running container uses this) ───────────────────
resource "aws_iam_role" "task" {
  name               = "${local.name_prefix}-task-role"
  assume_role_policy = data.aws_iam_policy_document.ecs_task_assume.json
}

# Extend this policy with any permissions your app needs at runtime
# (e.g. S3 read, SQS send, DynamoDB access)
